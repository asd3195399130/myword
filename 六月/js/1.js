...
(function ($) {
  ...
  justTools.prototype = {
      ...
      switch (this.set.gravity) {
        case "top":
          if (rightTmp > $(window).width()) {
            ...
            this.obj
              .find(".just-" + this.set.gravity)
              .css("left", this.obj.outerWidth() - wh.w / 2 + "px");
          } else if (leftTmp < 0) {
            setPos = {
              x: pos.x,
              y: pos.y - this.obj.outerHeight() - this.set.distance,
    ...
    setEvent: function () {
      ...
      //console.log(self.set.events)
      if (
        self.set.events == "mouseover" ||
        self.set.events == "onmouseover" ||
        self.set.events == "mouseenter"
      ) {
    ...
    addConfirm: function () {
      this.obj.append(
        "<div class='just-confirm'><button type='button' class='just-yes'>" +
          this.set.yes +
          "</button><button type='button' class='just-no'>" +
          this.set.no +
          "</button></div>"
      );
