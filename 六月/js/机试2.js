  ...
  const inputEventFunction = () => {
    ...
    if (keywords.length === 1 && keywords[0] === "") {
      resultContent.innerHTML =
        '<div id="no-result"><i class="fas fa-search fa-5x"></i></div>';
    } else if (resultItems.length === 0) {
      resultContent.innerHTML =
        '<div id="no-result"><i class="fas fa-box-open fa-5x"></i></div>';
    } else {
  ...
  const fetchData = () => {
    fetch(KEEP.hexo_config.root + searchPath)
      ...
      .then((res) => {
        ...
        noResultDom &&
          (noResultDom.innerHTML = '<i class="fas fa-search fa-5x"></i>');
      });
  };
