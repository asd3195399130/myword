  ...
  KEEP.utils = {
    ...
    styleHandleWhenScroll() {
      ...
      if (scrollTop > this.prevScrollValue && scrollTop > this.innerHeight) {
        ...
        if (this.isHeaderTransparent) {
          if (
            scrollTop <= this.headerWrapper_dom.getBoundingClientRect().height
          ) {
    ...
    registerWindowScroll() {
      window.addEventListener("scroll", () => {
        ...
        // TOC scroll handle
        if (
          KEEP.theme_config.toc.enable &&
          KEEP.utils.hasOwnProperty("findActiveIndexByTOC")
        ) {
    ...
    toggleShowToolsList() {
      ...
      document.addEventListener("click", () => {
        sideToolsListDom.classList.contains("show") &&
          sideToolsListDom.classList.remove("show");
      });
    },
    ...
    // init has TOC
    initHasToc() {
      const tocNavDoms = document.querySelectorAll(
