...
`Animation`与 `Transition` 的功能相同，都是通过改变元素的属性值来实现动画效果的它们的区别在于，使用 `Transition` 的功能时只能用指定属性的开始值和结束值，然后在这两个属性值之间使用平滑过渡的方式实现动画效果，因此不能实现比较复杂的动画效果。`Animation` 功能通过定义多个关键愤，以及定义每个关键愤中元素的属性值来实现更为复杂的动画效果。
